...
class Trainer:
    def __init__(self, params: configargparse.Namespace):
        ...
        if self.ngpu > 0:
            try:
                ...
                raise ValueError(f"{self.ngpu} > 0 but GPU is not detected.")
        ...
        bpe_temp = params.bpe
        params.bpe = None  # SPM model cannot be serialized
    ...
    def train(self):
        ...
        while self.epoch < self.nepochs:
            ...
            log_str = (
                f"Epoch {self.epoch:02d}, lr={self.opt.param_groups[0]['lr']} | "
                f"Train: loss={self.train_stats['loss']:.4f} | "
                f"Val: loss={self.val_stats['loss']:.4f} | "
    ...
    def train_epoch(self):
        ...
        for i, (feats, feat_lens, target, target_lens, train_keys) in enumerate(
            ...
            self.train_stats["loss"] += loss.item() * self.params.accum_grad
