...
class ASRDataset(Dataset):
    ...
    def tokenize_text(self, idx:str, text: str, mode:str):
        """
        :param str idx- Utterance ID key
        :param str text- ASR text transcript
        :returns np.array target- text tokenized into ints, text_lid - raw text which may have LID
        """
        # TODO add the language ID tag when we perform multilingual training
        # Hint: use the metadata in the utterance id
        # The tag is [ENG] for english, [ITA] for italian
        if mode == 'multilingual':
            # Extract language from utterance ID
            # Common patterns: idx contains "eng"/"english" or "ita"/"italian"
            idx_lower = idx.lower()
            if 'eng' in idx_lower or 'english' in idx_lower:
                text_lid = '[ENG] ' + text
            elif 'ita' in idx_lower or 'italian' in idx_lower:
                text_lid = '[ITA] ' + text
            else:
        ...
        target =  np.array(self.bpe.encode(text_lid))
        return target, text_lid
    ...
    def __getitem__(self, idx: str):
        ...
        audio_feat, sr = sf.read(self.data[idx]["input"][0]["feat"])
        audio_feat = torch.from_numpy(audio_feat).float() 

        feat_len = self.data[idx]["input"][0]["shape"][0]

        if "output" in self.data[idx].keys():
            target = self.data[idx]["output"][0]["tokenid"]
            target, _ = self.tokenize_text(idx, target, self.mode)
        else:
            target = None
