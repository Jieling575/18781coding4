...
class ASRModel(torch.nn.Module):
    ...
    def decode_greedy(self, xs, xlens):
        ...
        maxlen_out = maxlen + 50  # maximum decoding length
        
        # Initialize with <sos> token
        ys_in_pad = torch.full((batch_size, 1), self.sos, dtype=torch.long, device=xs.device)
        ys_in_lens = torch.ones(batch_size, dtype=torch.long, device=xs.device)
        cache = None
        
        # Store predictions for each sequence in the batch
        predictions = [[] for _ in range(batch_size)]
        finished = torch.zeros(batch_size, dtype=torch.bool, device=xs.device)
        
        for i in range(maxlen_out):
            # Forward one step
            y, cache = self.decoder.forward_one_step(
                encoder_out, encoder_out_lens, ys_in_pad, ys_in_lens, cache
            )
            
            # Greedy decoding: take the token with highest probability
            next_token = y.argmax(dim=-1)  # (batch,)
            
            # Update predictions for sequences that haven't finished
            for b in range(batch_size):
                if not finished[b]:
                    predictions[b].append(next_token[b].item())
                    if next_token[b].item() == self.eos:
                        finished[b] = True
            ...
            ys_in_pad = torch.cat([ys_in_pad, next_token], dim=1)
